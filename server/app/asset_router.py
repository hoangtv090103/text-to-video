...
async def _call_visual_service(service_func: Callable, visual_prompt: str, job_id: str, scene_id: int) -> str:
...
async def generate_visual_asset(scene: Dict, job_id: str = "unknown") -> Dict:
    ...
    logger.info("Starting visual asset generation", extra={
        "scene_id": scene_id,
    ...
    try:
        ...
        logger.info("Visual asset generation completed", extra={
            "scene_id": scene_id,
            "visual_type": visual_type,
        ...
        logger.error("Visual asset generation failed, using placeholder", extra={
            "scene_id": scene_id,
            "visual_type": visual_type,
            "error": str(e),
...
async def _create_error_placeholder(scene_id: str, visual_type: str, error: str) -> str:
    ...
    try:
        ...
        # Add error content
        ax.text(5, 4.5, f"⚠️ Scene {scene_id}", fontsize=20, fontweight='bold',
                ha='center', va='center', color='#c62828')
        ax.text(5, 3.5, "Visual Generation Failed", fontsize=16,
                ha='center', va='center', color='#c62828')
        ax.text(5, 2.8, f"Type: {visual_type}", fontsize=12,
                ha='center', va='center', color='#424242')

        # Truncate error message if too long
        error_text = error[:60] + "..." if len(error) > 60 else error
        ax.text(5, 2.2, f"Error: {error_text}", fontsize=10,
                ha='center', va='center', color='#424242', style='italic')

        # Add error border
        border = mpatches.Rectangle((0.5, 0.5), 9, 5, fill=False,
        ...
        logger.error("Failed to create error placeholder", extra={
            "scene_id": scene_id,
            "visual_type": visual_type,
