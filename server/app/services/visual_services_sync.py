...
def generate_visual_asset_sync(scene: Dict, job_id: str) -> Dict:
    ...
    logger.info("Starting synchronous visual asset generation", extra={
        "scene_id": scene_id,
        "job_id": job_id,
    ...
    try:
        ...
        logger.error("Visual asset generation failed", extra={
            "scene_id": scene_id,
            "job_id": job_id,
            "visual_type": visual_type,
            "error": str(e)
...
def _generate_slide_sync(visual_prompt: str, job_id: str, scene_id: int) -> Dict:
    ...
    try:
        ...
        logger.warning("Presenton API failed, using matplotlib fallback", extra={
            "scene_id": scene_id,
            "error": str(e)
        })
    ...
    logger.info("Slide generated successfully using matplotlib", extra={
        "scene_id": scene_id,
        "job_id": job_id,
...
def _generate_diagram_sync(visual_prompt: str, job_id: str, scene_id: int) -> Dict:
    ...
    # Title
    ax.text(5, 5.5, f"Diagram: Scene {scene_id}", fontsize=24, fontweight='bold',
    ...
    ax.text(5.5, 4, "Process", fontsize=12, ha='center', va='center', color='white', fontweight='bold')
    ...
    ax.text(8.75, 4, "Output", fontsize=12, ha='center', va='center', color='white', fontweight='bold')

    # Add description
    description_lines = visual_prompt.split('\n')[:2]
    for i, line in enumerate(description_lines):
        ax.text(5, 2.5 - i * 0.5, line[:60], fontsize=10, ha='center', va='center',
               color='#7f8c8d')
    ...
    logger.info("Diagram generated successfully", extra={
        "scene_id": scene_id,
        "job_id": job_id,
        "output_file": output_file
    })
...
def _generate_animation_sync(visual_prompt: str, job_id: str, scene_id: int) -> Dict:
    ...
    """
    output_file = os.path.join(
        ASSET_STORAGE_PATH, f"job_{job_id}_scene_{scene_id}_animation.png"
    )
    ...
    # Title
    ax.text(5, 5.5, f"Animation: Scene {scene_id}", fontsize=24, fontweight='bold',
    ...
    description_lines = visual_prompt.split('\n')[:2]
    for i, line in enumerate(description_lines):
        ax.text(5, 1.5 - i * 0.3, line[:60], fontsize=10, ha='center', va='center',
               color='#34495e')
    ...
    logger.info("Animation frame generated successfully", extra={
        "scene_id": scene_id,
        "job_id": job_id,
        "output_file": output_file
    })
...
def _generate_image_sync(visual_prompt: str, job_id: str, scene_id: int) -> Dict:
    ...
    """
    output_file = os.path.join(
        ASSET_STORAGE_PATH, f"job_{job_id}_scene_{scene_id}_image.png"
    )
    ...
    # Title
    ax.text(5, 5.5, f"Image: Scene {scene_id}", fontsize=24, fontweight='bold',
    ...
    description_lines = visual_prompt.split('\n')[:2]
    for i, line in enumerate(description_lines):
        ax.text(5, 0.8 - i * 0.3, line[:60], fontsize=10, ha='center', va='center',
    ...
    logger.info("Image generated successfully", extra={
        "scene_id": scene_id,
        "job_id": job_id,
        "output_file": output_file
    })
...
def _call_presenton_api_sync(visual_prompt: str, job_id: str, scene_id: int) -> Dict:
    ...
    """
    logger.info("Generating slide via Presenton API (sync)", extra={
        "scene_id": scene_id,
        "job_id": job_id
    })
    ...
    try:
        ...
        logger.info("Presenton API call successful, using placeholder", extra={
            "scene_id": scene_id,
            "job_id": job_id
        })
        ...
        return {
            "path": "",
            "status": "api_success_needs_conversion",
            "visual_type": "slide"
        }

    except Exception as e:
        logger.error("Presenton API call failed", extra={
            "scene_id": scene_id,
            "job_id": job_id,
            "error": str(e)
        })
